...
<div class="card">
  <div class="card-header">
    ...
    <div style="display:flex;gap:10px;align-items:center;">
      <a href="/public-booking" target="_blank" class="btn btn-outline btn-sm" title="Share this link with patients so they can book without an account">
        <i class="fas fa-link"></i> Public Booking Link
      </a>
  ...
  <div class="card-body">
    <table class="data-table">
      <thead><tr><th>#</th><th>Patient</th><th>Phone</th><th>Doctor</th><th>Date & Time</th><th>Status</th><th>Notes</th><th>Actions</th></tr></thead>
      <tbody id="app-body">
...
async function loadAppointments() {
  ...
  if (!data || !data.length) {
    tb.innerHTML='<tr><td colspan="8" style="text-align:center;padding:24px;color:#94a3b8;">No appointments found</td></tr>';
    return;
  }
  ...
  tb.innerHTML = data.map(a => {
    ...
    // Guest bookings show a yellow "Guest / Walk-in" badge
    const patientCell = a.is_guest
      ? `<span style="font-weight:600;">${a.display_name}</span><br>
         <span style="font-size:11px;background:#fef9c3;color:#854d0e;padding:2px 7px;border-radius:99px;font-weight:600;">
           <i class="fas fa-user-clock" style="font-size:9px;"></i> Guest / Walk-in
         </span>`
      : `<span style="font-weight:600;">${a.display_name}</span>`;

    const doctorName = a.doctor && a.doctor.user ? a.doctor.user.name : (a.doctor ? a.doctor.name : '—');
    const doctorSpec  = a.doctor ? (a.doctor.specialty || '') : '';

    return `<tr>
      <td style="color:#94a3b8;font-size:12px;">#${a.id}</td>
      <td>${patientCell}</td>
      <td style="font-size:13px;color:#64748b;">${a.display_phone || '—'}</td>
      <td>${doctorName} <span style="font-size:11px;color:#94a3b8;display:block">${doctorSpec}</span></td>
      ...
      <td>
        <button class="btn btn-sm" style="background:#f0fdf4;color:#166534;border:none;margin-right:4px;"
          onclick="confirmApp(${a.id})" title="Confirm appointment">
          <i class="fas fa-check"></i>
        </button>
        <button class="btn btn-sm" style="background:#fff1f2;color:#e94560;border:none;" onclick="deleteApp(${a.id})">
...
async function confirmApp(id) {
  await fetch(`/api/appointments/${id}`, {method:'PUT', headers: h, body: JSON.stringify({status:'confirmed'})});
  loadAppointments();
}
