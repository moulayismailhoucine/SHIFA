...
<aside class="sidebar" id="sidebar">
  ...
  <nav class="sidebar-nav">
    ...
    {{-- Nurse Portal (shown only for nurse role via JS) --}}
    <div class="nav-section-label nurse-only" style="display:none;">{{ __('Nurse Portal') }}</div>
    <div class="nav-item nurse-only" style="display:none;">
      <a href="/nurse/dashboard" class="nav-link {{ request()->is('nurse/dashboard') ? 'active' : '' }}" id="nav-nurse-dash">
        <span class="icon"><i class="fas fa-th-large"></i></span> {{ __('My Dashboard') }}
      </a>
    </div>
    <div class="nav-item nurse-only" style="display:none;">
      <a href="/nurse/patients" class="nav-link {{ request()->is('nurse/patients') ? 'active' : '' }}" id="nav-nurse-patients">
        <span class="icon"><i class="fas fa-user-injured"></i></span> {{ __('My Patients') }}
      </a>
    </div>
    <div class="nav-item nurse-only" style="display:none;">
      <a href="/nurse/orders" class="nav-link {{ request()->is('nurse/orders') ? 'active' : '' }}" id="nav-nurse-orders">
        <span class="icon"><i class="fas fa-clipboard-list"></i></span> {{ __('Nursing Orders') }}
      </a>
    </div>

    <div class="nav-section-label">{{ __('Administration') }}</div>
    <div class="nav-item doctor-only" style="display:none;">
      <a href="/doctor/nursing-tasks" class="nav-link {{ request()->is('doctor/nursing-tasks') ? 'active' : '' }}" id="nav-doctor-nursing-tasks">
        <span class="icon"><i class="fas fa-tasks"></i></span> {{ __('Nursing Tasks') }}
      </a>
    </div>
...
<div class="main">
  <header class="topbar" style="background: var(--card);">
    ...
    <div class="topbar-right">
      ...
      <!-- Notification Bell (Admin: shows unread contact messages) -->
      <div style="position:relative;" id="notif-wrapper">
        <button class="topbar-btn" id="notif-bell-btn" title="Notifications"
          ...
          <span id="notif-badge" style="display:none;position:absolute;top:4px;right:4px;
            background:var(--accent);color:white;border-radius:50%;width:18px;height:18px;
            font-size:10px;font-weight:700;display:none;align-items:center;justify-content:center;
            border:2px solid white;"></span>
        </button>
        <!-- Dropdown -->
        <div id="notif-dropdown" style="display:none;position:absolute;top:48px;right:0;
             background:white;border-radius:14px;box-shadow:0 8px 40px rgba(0,0,0,0.18);
             width:340px;z-index:200;border:1px solid var(--border);overflow:hidden;">
          <div style="padding:14px 18px;border-bottom:1px solid var(--border);display:flex;
            ...
            <a href="/admin/contact-messages" style="font-size:12px;color:var(--accent);
               text-decoration:none;font-weight:600;">View all</a>
          </div>
          <div id="notif-list" style="max-height:300px;overflow-y:auto;"></div>
          <div id="notif-empty" style="padding:28px;text-align:center;color:var(--text-muted);
  ...
  if (user) {
    try {
      ...
      if (user.role === 'admin') {
        ...
        document.querySelectorAll('.doctor-only').forEach(el => el.style.display = '');
        ...
        document.querySelectorAll('#nav-appointments')
          .forEach(el => el.parentElement.style.display = 'none');
        // Hide Administration section items
        document.querySelectorAll('#nav-doctors, #nav-pharmacies, #nav-laboratories')
          .forEach(el => el.parentElement.style.display = 'none');
        // Show nurse portal navigation
        document.querySelectorAll('.nurse-only').forEach(el => el.style.display = '');
        // Hide generic patients link (use nurse-specific one)
        const navPat = document.getElementById('nav-patients');
        if (navPat) navPat.closest('.nav-item').style.display = 'none';
      } else {
...
function logout() {
  ...
  // Hit /logout to clear any PHP web session too
...
(function initNotifications() {
  ...
  if (!user || user.role !== 'admin') { wrapper.style.display = 'none'; return; }
  ...
  async function fetchNotifications() {
    try {
      ...
      const navBadge = document.getElementById('nav-contact-badge');
      if (navBadge) {
      ...
      if (recent.length > 0) {
        ...
        list.innerHTML = recent.map(m => `
          <a href="/admin/contact-messages/${m.id}" style="display:block;padding:12px 18px;
             border-bottom:1px solid #f1f5f9;text-decoration:none;background:white;"
             onmouseover="this.style.background='#f8fafc'" onmouseout="this.style.background='white'">
            <div style="display:flex;align-items:center;gap:10px;">
              <div style="width:34px;height:34px;border-radius:10px;background:linear-gradient(135deg,#e94560,#f43f5e);
                          color:white;display:flex;align-items:center;justify-content:center;font-size:13px;flex-shrink:0;">
                <i class="fas fa-envelope"></i>
              </div>
              <div style="flex:1;min-width:0;">
                <div style="font-weight:600;font-size:12.5px;color:#1e293b;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">
                  ${m.name}
                </div>
              ...
              <span style="background:#e94560;color:white;border-radius:10px;padding:2px 7px;font-size:10px;font-weight:700;flex-shrink:0;">NEW</span>
            </div>
          </a>`).join('');
      } else {
