      ...
      data.map(res => `
        <div style="border:1.5px solid #e2e8f0;border-radius:14px;overflow:hidden;background:#f8fafc;">
          ...
          <div style="padding:12px;">
            ...
            ${res.note ? `<div style="font-size:12px;color:#64748b;margin-top:4px;white-space:pre-wrap;">${res.note}</div>` : ''}
...
async function loadVitals() {
  ...
  if (!vitals.length) {
    const emptyMsg = currentLang === 'ar' ? 'لا توجد قراءات حيوية حتى الآن' : 'No vitals recorded yet to show a curve.';
    container.innerHTML = `<div style="text-align:center;padding:24px;color:#94a3b8;">${emptyMsg}</div>`;
    return;
  }

  container.innerHTML = '<canvas id="patientVitalsChart" style="width:100%;max-height:300px;"></canvas>';
  ...
  vitals.forEach(v => {
    ...
    if (v.blood_pressure && v.blood_pressure.includes('/')) {
      ...
      sys = v.blood_pressure_systolic || null;
      dia = v.blood_pressure_diastolic || null;
    }
    systolic.push(sys);
    diastolic.push(dia);
  ...
  new Chart(ctx, {
    ...
    data: {
      labels: labels,
      datasets: [
        { label: currentLang === 'ar' ? 'ضغط الدم الانقباضي' : 'Systolic BP', data: systolic, borderColor: '#ef4444', backgroundColor: 'rgba(239, 68, 68, 0.1)', tension: 0.3, fill: false },
        { label: currentLang === 'ar' ? 'ضغط الدم الانبساطي' : 'Diastolic BP', data: diastolic, borderColor: '#3b82f6', backgroundColor: 'rgba(59, 130, 246, 0.1)', tension: 0.3, fill: false },
        { label: currentLang === 'ar' ? 'معدل ضربات القلب' : 'Heart Rate', data: hr, borderColor: '#10b981', backgroundColor: 'rgba(16, 185, 129, 0.1)', tension: 0.3, fill: false },
        { label: currentLang === 'ar' ? 'درجة الحرارة (°C)' : 'Temperature (°C)', data: temp, borderColor: '#f59e0b', backgroundColor: 'rgba(245, 158, 11, 0.1)', tension: 0.3, fill: false }
      ]
    },
    options: {
      responsive: true,
