...
<div id="profile-content" style="display:none;">
  ...
  <div style="display:flex;flex-wrap:wrap;gap:10px;margin-bottom:20px;" id="tab-buttons">
    ...
    <button class="btn btn-outline btn-sm" onclick="switchTab('nursing', this)"><i class="fas fa-user-nurse"></i> Nursing & Vitals</button>
  </div>
  ...
  <div id="tab-records" class="card">
    <div class="card-header">
      ...
      <button onclick="generateAiSummary()" id="ai-summary-btn"
        style="margin-left:auto;background:linear-gradient(135deg,#065f46,#10b981);color:white;border:none;padding:8px 18px;border-radius:10px;font-size:13px;font-weight:700;cursor:pointer;display:flex;align-items:center;gap:8px;">
        <i class="fas fa-brain"></i> AI Medication Summary
      </button>
    </div>

    {{-- AI Summary Panel (hidden until generated) --}}
    <div id="ai-summary-panel" style="display:none;margin:16px 20px 0;border:2px solid #10b981;border-radius:14px;overflow:hidden;">
      <div style="background:linear-gradient(135deg,#065f46,#10b981);padding:12px 18px;display:flex;align-items:center;gap:10px;">
        <i class="fas fa-robot" style="color:white;font-size:18px;"></i>
        <span style="color:white;font-weight:700;font-size:14px;">AI Medication Summary</span>
        <span id="ai-method-badge" style="margin-left:auto;background:rgba(255,255,255,0.2);color:white;padding:3px 10px;border-radius:99px;font-size:11px;"></span>
        <button onclick="document.getElementById('ai-summary-panel').style.display='none'"
          style="background:none;border:none;color:rgba(255,255,255,0.7);font-size:16px;cursor:pointer;margin-left:6px;">&times;</button>
      </div>
      <div style="background:white;padding:18px;">
        {{-- Summary text --}}
        <div id="ai-summary-text" style="font-size:14px;line-height:1.75;color:#1e293b;margin-bottom:14px;"></div>

        {{-- Interaction warnings --}}
        <div id="ai-warnings-box" style="display:none;">
          <div style="font-size:11px;font-weight:700;text-transform:uppercase;color:#92400e;letter-spacing:.5px;margin-bottom:6px;">⚠️ Drug Interaction Alerts</div>
          <div id="ai-warnings-list"></div>
        </div>

        {{-- Drug cards --}}
        <div id="ai-drugs-box" style="display:none;">
          <div style="font-size:11px;font-weight:700;text-transform:uppercase;color:#64748b;letter-spacing:.5px;margin:10px 0 8px;">💊 Drug Details (RxNorm API)</div>
          <div id="ai-drugs-list" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(260px,1fr));gap:10px;"></div>
        </div>
      </div>
    </div>
  ...
  {{-- Nursing & Vitals Tab --}}
  <div id="tab-nursing" class="card" style="display:none;">
    <div class="card-header">
      <span class="card-title"><i class="fas fa-user-nurse" style="color:#10b981;margin-right:8px;"></i>Nursing & Vitals</span>
    </div>
    <div class="card-body" style="padding:20px;">
      <div style="margin-bottom:30px;">
        <h4 style="margin-bottom:15px;color:#0f3460;"><i class="fas fa-chart-line"></i> Vitals History Curve</h4>
        <canvas id="vitalsChart" style="width:100%;max-height:300px;"></canvas>
      </div>
      <div>
        <h4 style="margin-bottom:15px;color:#0f3460;"><i class="fas fa-clipboard-list"></i> Nursing Orders & Procedures</h4>
        <div id="nursing-body">
          <div style="text-align:center;padding:24px;color:#94a3b8;"><i class="fas fa-spinner fa-spin"></i></div>
        </div>
      </div>
      <div style="margin-top:30px;">
        <h4 style="margin-bottom:15px;color:#0f3460;"><i class="fas fa-notes-medical"></i> Nurse Notes</h4>
        <div id="nurse-notes-body">
          <div style="text-align:center;padding:24px;color:#94a3b8;"><i class="fas fa-spinner fa-spin"></i></div>
        </div>
      </div>
    </div>
...
async function init() {
  try {
    ...
    let allVitals = [...(patientData.vitals || []), ...(patientData.vitalSigns || [])]
      .sort((a,b) => new Date(a.created_at) - new Date(b.created_at));
      
    renderNursingAndVitals(patientData.nursingOrders || [], allVitals, patientData.nurseNotes || []);
...
function renderLabNote(note) {
  ...
  // ── Skin Cancer AI block ──────────────────────────────────
  if (note.includes('=== AUTOMATIC AI ANALYSIS ===')) {
    const probMatch = note.match(/Risk Probability:\s*([\d.]+)%/);
    const diagMatch = note.match(/Suspected Diagnosis:\s*(\w+)/);
    ...
    const icon      = isHigh ? '⚠️' : '✅';
    const afterAI   = note.split('===========================').pop().replace(/^\n+/, '').replace('Doctor Notes:', '').trim();
    return `
      <div style="margin-top:8px;border:1.5px solid ${border};border-radius:10px;background:${bg};padding:10px 12px;">
        <div style="font-weight:800;font-size:12px;color:${color};margin-bottom:4px;">${icon} AI SKIN CANCER ANALYSIS</div>
        <div style="display:flex;gap:12px;flex-wrap:wrap;">
          <div style="font-size:13px;color:${color};"><strong>Diagnosis:</strong> ${diag}</div>
          <div style="font-size:13px;color:${color};"><strong>Risk:</strong> ${prob}%</div>
        </div>
        ${prob !== null ? `<div style="margin-top:6px;height:6px;background:#e5e7eb;border-radius:99px;overflow:hidden;"><div style="height:100%;width:${prob}%;background:${color};border-radius:99px;"></div></div>` : ''}
        ${afterAI ? `<div style="margin-top:6px;font-size:11px;color:#64748b;"><strong>Notes:</strong> ${afterAI}</div>` : ''}
      </div>`;
  }

  // ── Fracture AI block ─────────────────────────────────────
  if (note.includes('=== AUTOMATIC FRACTURE ANALYSIS ===')) {
    const probMatch  = note.match(/Fracture Probability:\s*([\d.]+)%/);
    const diagMatch  = note.match(/Diagnosis:\s*(\w+)/);
    const noteMatch  = note.match(/Clinical Note:\s*(.+?)(?=\n)/s);
    const prob       = probMatch ? parseFloat(probMatch[1]) : null;
    const diag       = diagMatch ? diagMatch[1] : 'Unknown';
    const clinical   = noteMatch ? noteMatch[1].trim() : '';
    const isFractured = diag === 'Fractured';
    const color  = isFractured ? '#d97706' : '#0369a1';
    const bg     = isFractured ? '#fffbeb' : '#eff6ff';
    const border = isFractured ? '#fcd34d' : '#93c5fd';
    const icon   = isFractured ? '🦴' : '✅';
    const afterAI = note.split('===================================').pop().replace(/^\n+/, '').replace('Doctor Notes:', '').trim();
    return `
      <div style="margin-top:8px;border:1.5px solid ${border};border-radius:10px;background:${bg};padding:10px 12px;">
        <div style="font-weight:800;font-size:12px;color:${color};margin-bottom:4px;">${icon} AI X-RAY FRACTURE ANALYSIS</div>
        <div style="display:flex;gap:12px;flex-wrap:wrap;">
          <div style="font-size:13px;color:${color};"><strong>Diagnosis:</strong> ${diag}</div>
          <div style="font-size:13px;color:${color};"><strong>Probability:</strong> ${prob}%</div>
        </div>
        ${prob !== null ? `<div style="margin-top:6px;height:6px;background:#e5e7eb;border-radius:99px;overflow:hidden;"><div style="height:100%;width:${prob}%;background:${color};border-radius:99px;"></div></div>` : ''}
        ${clinical ? `<div style="margin-top:8px;background:white;border-radius:8px;padding:8px 10px;font-size:12px;color:#334155;border-left:3px solid ${color};"><strong>📋 Clinical Note:</strong> ${clinical}</div>` : ''}
        ${afterAI ? `<div style="margin-top:6px;font-size:11px;color:#64748b;"><strong>Notes:</strong> ${afterAI}</div>` : ''}
      </div>`;
  }

  // ── Plain note ────────────────────────────────────────────
  return `<div style="font-size:11.5px;color:#64748b;margin-top:4px;border-left:3px solid #f59e0b;padding-left:8px;white-space:pre-wrap;">${note}</div>`;
}

// ── AI Medication Summary (calls Laravel which calls Python NLP + RxNorm API) ──
async function generateAiSummary() {
  ...
  try {
    // Use FormData to POST to the medication-summary route
    const fd = new FormData();
    fd.append('patient_id', patientId);
    fd.append('_token', document.querySelector('meta[name="csrf-token"]')?.content || '');

    const resp = await fetch('/medication-summary/api', {
      method: 'POST',
    ...
    if (data.success) {
      ...
      // Summary text
      document.getElementById('ai-summary-text').textContent = r.summary;
      document.getElementById('ai-method-badge').textContent = r.nlp_method;

      // Warnings
      if (r.interaction_warnings && r.interaction_warnings.length) {
        document.getElementById('ai-warnings-box').style.display = 'block';
        document.getElementById('ai-warnings-list').innerHTML = r.interaction_warnings
          .map(w => `<div style="background:#fffbeb;border:1px solid #fcd34d;border-radius:8px;padding:8px 12px;font-size:12.5px;color:#92400e;margin-bottom:6px;">${w}</div>`).join('');
      } else {
        document.getElementById('ai-warnings-box').style.display = 'none';
      }

      // Drug cards
      if (r.drug_details && r.drug_details.length) {
        document.getElementById('ai-drugs-box').style.display = 'block';
        document.getElementById('ai-drugs-list').innerHTML = r.drug_details.map(d => `
          <div style="background:#f8fafc;border:1.5px solid #e2e8f0;border-radius:10px;padding:12px;">
            <div style="font-weight:700;color:#0f3460;font-size:13px;">${d.name}</div>
            <span style="display:inline-block;background:#ede9fe;color:#6d28d9;padding:2px 8px;border-radius:99px;font-size:11px;font-weight:600;margin:4px 0;">${d.drug_class}</span>
            ${d.rxcui !== 'N/A' ? `<span style="display:inline-block;background:#f0f9ff;color:#0369a1;padding:2px 8px;border-radius:99px;font-size:11px;font-weight:600;margin:4px 0 4px 4px;">RxCUI: ${d.rxcui}</span>` : ''}
            <div style="font-size:12px;color:#334155;margin-top:4px;"><strong>Purpose:</strong> ${d.purpose}</div>
            <div style="font-size:11.5px;color:#64748b;margin-top:2px;"><strong>Monitor:</strong> ${d.side_effects}</div>
          </div>`).join('');
      }

      panel.style.display = 'block';
    } else {
      alert(data.message || 'No medication history found for this patient.');
    }
  } catch (e) {
    alert('Failed to generate summary. Please try again.');
  } finally {
    btn.innerHTML = '<i class="fas fa-brain"></i> AI Medication Summary';
    btn.disabled = false;
  }
}
...
function switchTab(name, btn) {
  ['records','prescriptions','appointments','labresults','nursing'].forEach(t => {
...
function renderNursingAndVitals(orders, vitals, notes) {
  // Render Nursing Orders
  const container = document.getElementById('nursing-body');
  if (!orders.length) {
    container.innerHTML = '<div style="text-align:center;padding:24px;color:#94a3b8;">No nursing orders found.</div>';
  } else {
    container.innerHTML = orders.map(o => `
      <div style="border:1.5px solid #e2e8f0;border-radius:14px;padding:16px;margin-bottom:12px;background:#f8fafc;">
        <div style="display:flex;justify-content:space-between;margin-bottom:8px;">
          <div style="font-weight:700;color:#0f3460;font-size:14px;">${o.type} <span style="font-weight:400;color:#64748b;font-size:12px;">— Executed by ${o.nurse?.name || 'Unknown'}</span></div>
          <span class="badge ${o.status==='Completed'?'badge-success':o.status==='Ongoing'?'badge-info':'badge-warning'}">${o.status}</span>
        </div>
        <div style="font-size:12px;color:#374151;"><strong>Task:</strong> ${o.instructions || o.notes || '—'}</div>
        ${o.result ? `<div style="font-size:12px;color:#166534;background:#f0fdf4;padding:8px;border-radius:6px;margin-top:8px;"><strong>Result:</strong> ${o.result}</div>` : ''}
        <div style="font-size:11px;color:#94a3b8;margin-top:8px;"><i class="fas fa-clock"></i> ${new Date(o.created_at).toLocaleString()}</div>
      </div>
    `).join('');
  }

  // Render Nurse Notes
  const notesContainer = document.getElementById('nurse-notes-body');
  if (!notes || !notes.length) {
    notesContainer.innerHTML = '<div style="text-align:center;padding:24px;color:#94a3b8;">No nurse notes found.</div>';
  } else {
    notesContainer.innerHTML = notes.map(n => `
      <div style="border:1.5px solid #fcd34d;border-radius:14px;padding:16px;margin-bottom:12px;background:#fffbeb;">
        <div style="display:flex;justify-content:space-between;margin-bottom:8px;">
          <div style="font-weight:700;color:#92400e;font-size:14px;text-transform:capitalize;">${n.type || 'Observation'} Note</div>
          ${n.pain_level ? `<span class="badge" style="background:#ef4444;color:white;">Pain: ${n.pain_level}/10</span>` : ''}
        </div>
        <div style="font-size:13px;color:#92400e;line-height:1.5;">${n.note}</div>
        <div style="font-size:11px;color:#b45309;margin-top:8px;"><i class="fas fa-clock"></i> ${new Date(n.created_at).toLocaleString()}</div>
      </div>
    `).join('');
  }

  // Render Vitals Chart (if we have vitals)
  const ctx = document.getElementById('vitalsChart').getContext('2d');
  if (!vitals.length) {
    ctx.canvas.parentElement.innerHTML = '<div style="text-align:center;padding:24px;color:#94a3b8;">No vitals recorded yet to show a curve.</div>';
    return;
  }

  // Parse vitals
  const labels = vitals.map(v => new Date(v.created_at).toLocaleDateString() + ' ' + new Date(v.created_at).getHours() + ':00');
  ...
  vitals.forEach(v => {
    ...
    } else if (v.blood_pressure && v.blood_pressure.includes('/')) {
    ...
    systolic.push(sys);
    diastolic.push(dia);
    temp.push(v.temperature || null);
    hr.push(v.heart_rate || null);
  });
  ...
  vitalsChartInstance = new Chart(ctx, {
    ...
    data: {
      labels: labels,
      datasets: [
        { label: 'Systolic BP', data: systolic, borderColor: '#ef4444', backgroundColor: 'rgba(239, 68, 68, 0.1)', tension: 0.3, fill: false },
        { label: 'Diastolic BP', data: diastolic, borderColor: '#3b82f6', backgroundColor: 'rgba(59, 130, 246, 0.1)', tension: 0.3, fill: false },
        { label: 'Heart Rate', data: hr, borderColor: '#10b981', backgroundColor: 'rgba(16, 185, 129, 0.1)', tension: 0.3, fill: false },
        { label: 'Temperature (°C)', data: temp, borderColor: '#f59e0b', backgroundColor: 'rgba(245, 158, 11, 0.1)', tension: 0.3, fill: false }
      ]
    },
    options: {
      responsive: true,
