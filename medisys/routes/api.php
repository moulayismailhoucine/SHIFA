...
Route::middleware(['auth:sanctum'])->group(function () {
    ...
    // Nursing Orders — strict RBAC enforced in controller
    Route::apiResource('nursing-orders', App\Http\Controllers\Api\NursingOrderController::class);
    Route::get('/nurse/my-patients', [App\Http\Controllers\Api\NursingOrderController::class, 'myPatients']);
    Route::get('/nurse/my-nurses',   [App\Http\Controllers\Api\NursingOrderController::class, 'myNurses']);
    Route::get('/nurse/audit-log',   [App\Http\Controllers\Api\NursingOrderController::class, 'auditLog']);
    ...
    // AI Diagnosis Analyses
    Route::get('/ai-analyses', [AiAnalysisController::class, 'index']);
    Route::get('/ai-analyses/{aiAnalysis}', [AiAnalysisController::class, 'show']);
    Route::post('/ai-analyses', [AiAnalysisController::class, 'store']);
    Route::put('/ai-analyses/{aiAnalysis}', [AiAnalysisController::class, 'update']);
    Route::delete('/ai-analyses/{aiAnalysis}', [AiAnalysisController::class, 'destroy']);
    Route::get('/ai-health', [AiAnalysisController::class, 'health']);

    // ML Skin Cancer Analysis
    Route::post('/skin-cancer/analyze', [\App\Http\Controllers\Api\SkinCancerAnalysisController::class, 'analyze']);
    ...
    // Admin notifications (unread contact messages count)
    Route::get('/admin/notifications', function () {
        $unread = \App\Models\ContactMessage::where('status', 'new')->count();
        $recent = \App\Models\ContactMessage::where('status', 'new')
            ->orderBy('created_at', 'desc')
            ->limit(5)
            ->get(['id', 'name', 'subject', 'created_at']);
        return response()->json([
            'success' => true,
            'data'    => ['unread' => $unread, 'recent' => $recent],
        ]);
