...
class NurseController extends Controller
{
    ...
    {
        $patients = Patient::orderBy('created_at', 'desc')->paginate(12);
        return view('nurse.patients', compact('patients'));
    }

    public function orders()
    {
        return view('nurse.orders.index');
    }
    ...
    public function storeVitals(Request $request, Patient $patient)
    {
        ...
        // (Optional AI feature): Detect abnormal vital signs and generate alerts
        $abnormal = [];
        if ($vital->temperature > 38.0 || $vital->temperature < 35.0) $abnormal[] = 'Temperature: ' . $vital->temperature;
        if ($vital->heart_rate > 100 || $vital->heart_rate < 60) $abnormal[] = 'Heart Rate: ' . $vital->heart_rate;
        if ($vital->blood_pressure_systolic > 140 || $vital->blood_pressure_systolic < 90) $abnormal[] = 'Systolic BP: ' . $vital->blood_pressure_systolic;
        if ($vital->blood_pressure_diastolic > 90 || $vital->blood_pressure_diastolic < 60) $abnormal[] = 'Diastolic BP: ' . $vital->blood_pressure_diastolic;
        if ($vital->oxygen_saturation < 95) $abnormal[] = 'SpO2: ' . $vital->oxygen_saturation . '%';

        if (count($abnormal) > 0) {
            \App\Models\Alert::create([
                'user_id' => Auth::id(), // or the assigned doctor's ID
                'patient_id' => $patient->id,
                'title' => 'Critical Vitals Alert - ' . $patient->name,
                'message' => 'Abnormal vitals detected: ' . implode(', ', $abnormal),
                'type' => 'critical',
                'is_read' => false,
            ]);
        }
        ...
        return redirect()->back()
            ->with('success', 'Vitals recorded successfully for ' . $patient->name . (count($abnormal) > 0 ? ' (Critical alert generated!)' : ''));
    }
    ...
    public function storeNote(Request $request, Patient $patient)
    {
        ...
        if ($request->hasFile('attachment')) {
            $attachmentPath = $request->file('attachment')->store('nurse_notes', 'public');
        }
