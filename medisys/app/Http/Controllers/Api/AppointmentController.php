...
class AppointmentController extends Controller
{
    public function index(Request $request)
    {
        ...
        $appointments = $query->orderBy('scheduled_at')->get()->map(function ($a) {
            $arr = $a->toArray();
            // Unified display name: prefer registered patient, fall back to guest
            $arr['display_name']  = $a->patient ? $a->patient->name  : ($a->guest_name  ?? 'Guest');
            $arr['display_phone'] = $a->patient ? ($a->patient->phone ?? '—') : ($a->guest_phone ?? '—');
            $arr['is_guest']      = is_null($a->patient_id);
            return $arr;
        });
    ...
    public function store(Request $request)
    {
        ...
        if ($doc) {
            ...
            if ($doc->break_start && $doc->break_end && $time >= $doc->break_start && $time < $doc->break_end) {
                return response()->json(['success' => false, 'message' => "The doctor is on break during this time."], 422);
            }
        }
    ...
    public function patientBook(Request $request)
    {
        ...
        if ($doc->break_start && $doc->break_end && $time >= $doc->break_start && $time < $doc->break_end) return response()->json(['success'=>false, 'message'=>"Doctor is on break"], 422);
    ...
    public function getAvailableSlots(Request $request)
    {
        ...
        while ($current < $last) {
            $t = date('H:i', $current);
            $endTime = date('H:i', strtotime("+{$step} minutes", $current));
            
            $isBreak = false;
            if ($doc->break_start && $doc->break_end) {
                $bStart = substr($doc->break_start, 0, 5);
                $bEnd = substr($doc->break_end, 0, 5);
                // If the slot starts during break, or overlaps with break
                if ($t >= $bStart && $t < $bEnd) {
                    $isBreak = true;
                }
            }

            if (!$isBreak && !in_array($t, $existing) && $current >= time()) {
                $slots[] = $t;
            }
        ...
        if (empty($slots)) {
            ...
            for ($i = 1; $i <= 14; $i++) {
                ...
                while ($nCur < $nLast) {
                    ...
                    $isNextBreak = false;
                    if ($doc->break_start && $doc->break_end) {
                        $nbStart = substr($doc->break_start, 0, 5);
                        $nbEnd = substr($doc->break_end, 0, 5);
                        if ($nt >= $nbStart && $nt < $nbEnd) {
                            $isNextBreak = true;
                        }
                    }

                    if (!$isNextBreak && !in_array($nt, $nextExisting)) {
